...
static uint8_t FraiseID = FRAISE_ID; // default Fraise ID is 10
...
void fraise_setup(/*bool background_rx*/) {
    ...
    //irq_set_exclusive_handler(pio_irq, fraise_irq);
...
{
    *rxpin = FRAISE_RX_PIN;//(watchdog_hw->scratch[7] >> 10) & 31;
    *txpin = FRAISE_TX_PIN;//(watchdog_hw->scratch[7] >> 5) & 31;
    *drvpin = FRAISE_DRV_PIN;//(watchdog_hw->scratch[7] >> 0) & 31;
    *drvlevel = FRAISE_DRV_LEVEL;//(watchdog_hw->scratch[7] >> 15) & 1;
}
